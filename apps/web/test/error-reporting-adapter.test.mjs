...
test("default error reporter is a no-op sink and returns sanitized metadata", async () => {
  ...
  const result = await errorReporter.rubyWhisperNoopErrorReporter.reportError({
    ...
    authMaterial: "payload must not echo",
    error: {
      code: "provider_error",
    ...
    metadata: {
      ...
      release: "rw-web-test",
      requestBody: "payload must not echo",
      runtime: "nodejs",
      screenshot: "payload must not echo",
      sessionReplay: "payload must not echo",
      totalLatencyMs: 420,
    },
    providerResponseBody: "provider response body",
    responseBody: "payload must not echo",
