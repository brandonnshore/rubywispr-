...
- `src/app/api/desktop/transcribe/route.ts` owns the RW-041 preflight and mocked
  provider execution path for desktop transcription. It authenticates Clerk
  users, checks Terms/Privacy, reads subscription and usage metadata, enforces
  quota entitlement, maps parser and provider failures to shared desktop API
  errors, calls an injected provider client with normalized in-memory audio, and
  returns provider text only for cleanup-disabled requests until RW-042 lands.
...
- Desktop transcription parser helpers such as
  ...
  provider payloads.
- The desktop transcription route is covered by
  ...
  `no-store`, provider work must use injected mocks in tests, cleanup-enabled
  success stays fail-closed until RW-042, and live provider calls remain out of
