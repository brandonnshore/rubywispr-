...
Friend of Ruby mocked validation is offline-only. `test/friend-of-ruby-mocked-integration.test.mjs` composes the admin batch route, Friend of Ruby batch metadata helper, mocked Stripe coupon/promotion-code adapter, Stripe subscription-cache mapper, account subscription-cache reader, and quota entitlement service using synthetic Stripe and Supabase clients. The focused helper tests remain offline-only too: `test/admin-friend-of-ruby-batches.test.mjs`, `test/friend-of-ruby-batches.test.mjs`, `test/friend-of-ruby-stripe.test.mjs`, `test/stripe-subscription-cache.test.mjs`, and `test/subscription-cache.test.mjs`.

The Friend of Ruby proof path must stay metadata-only and server-only: admin-created batches store only creator Clerk user ID, normalized batch code, max redemptions, optional expiration, and optional Stripe promotion code ID; mocked Stripe creation receives only batch code/id metadata; redemption simulation maps that safe metadata to subscription-cache `friend_of_ruby_until` state; account/quota reads expose only entitlement and usage metadata. The mocked fixtures reject private content fields and live-looking credentials. Live Stripe promotion-code redemption smoke remains blocked in RUB-197 and must not be added to autonomous validation without approved test-mode setup.

RUB-105 can depend on the completed RW-029A-E mocked Friend of Ruby backend leaves for offline account billing/admin integration prerequisites. It does not need to depend on broad RUB-40 completion for this mocked Friend of Ruby evidence path; live Stripe validation remains the separate RUB-197/RW-029F track.
